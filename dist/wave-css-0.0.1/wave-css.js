  ...
  class WcEmoji extends WcBaseComponent {
    ...
    static get defaultCategories() {
      return [
        {
          name: "Smileys",
          emojis: ["\u{1F600}", "\u{1F603}", "\u{1F604}", "\u{1F601}", "\u{1F602}", "\u{1F923}", "\u{1F60A}", "\u{1F607}", "\u{1F60D}", "\u{1F929}", "\u{1F618}", "\u{1F970}", "\u{1F60E}", "\u{1F914}", "\u{1FAE1}", "\u{1F917}", "\u{1F610}", "\u{1F611}", "\u{1F644}", "\u{1F62E}", "\u{1F622}", "\u{1F62D}"]
        },
        {
          name: "Gestures",
          emojis: ["\u{1F44D}", "\u{1F44E}", "\u{1F44F}", "\u{1F64C}", "\u{1F91D}", "\u{1F4AA}", "\u{1F91E}", "\u270C\uFE0F", "\u{1F44B}", "\u{1FAF6}"]
        },
        {
          name: "Symbols",
          emojis: ["\u2764\uFE0F", "\u{1F525}", "\u2B50", "\u2705", "\u{1F389}", "\u{1F4AF}", "\u{1F680}", "\u{1F4A1}", "\u26A1", "\u{1F440}", "\u{1F3C6}", "\u{1F48E}"]
        }
      ];
    }
    constructor() {
    ...
    _createInnerElement() {
      ...
      if (!pickerOnly) {
        let quickEmojis = ["\u{1F44D}", "\u2764\uFE0F", "\u{1F389}", "\u{1F602}"];
        const quickAttr = this.getAttribute("quick-emojis");
        if (quickAttr) {
          try {
            quickEmojis = JSON.parse(quickAttr);
          } catch (e) {
          }
        }
...
// src/js/components/wc-live-designer.js
  ...
  class WcLiveDesigner extends WcBaseComponent {
    ...
    static PRESETS = [
      // --- Page Starters ---
      {
        ...
        category: "page",
        description: "Skeleton + nav + tabs + form + hotkey",
  ...
  <wc-tab class="col-1 mt-2 mb-4" animate>
    ...
    <wc-tab-item label="Change Log">
      ...
      },
      {
        ...
        category: "page",
        description: "Skeleton + nav + form (no tabs)",
        html: `<wc-article-skeleton _="on load
      call WaveHelpers.waitForThenHideAndShow('wc-article-skeleton .wc-article-skeleton', '.page-content', 3000, 500)
    end"></wc-article-skeleton>
<div class="page-content flex flex-col flex-1 py-2 px-3 gap-2 hidden">
  ...
  <wc-form class="col gap-3 pt-2 pb-5 px-5" method="{{FormMethod}}" id="{{Template.Slug}}" action="/{{Template.RoutePrefix}}/{{Template.Slug}}/{{RecordID}}">
    <wc-hotkey keys="ctrl+s" target="button.save-btn"></wc-hotkey>
      ...
      },
      {
        label: "List Page",
        category: "page",
        description: "Skeleton + breadcrumb + tabulator",
        html: `<wc-article-skeleton _="on load
      call WaveHelpers.waitForThenHideAndShow('wc-article-skeleton .wc-article-skeleton', '.page-content', 3000, 500)
    end"></wc-article-skeleton>
<div class="page-content flex flex-col flex-1 py-2 px-3 gap-2 hidden">
  ...
  <wc-tabulator ajax-url="/api/{{Template.CollectionName}}" pagination></wc-tabulator>
      ...
      // --- Component Groups ---
      {
        ...
        category: "group",
        description: "Breadcrumb + save button row",
  ...
  <div class="flex flex-row items-center gap-3">
    <wc-save-split-button method="{{FormMethod}}" hx-include="form#{{Template.Slug}}" save-url="/{{Template.RoutePrefix}}/{{Template.Slug}}/{{RecordID}}" save-new-url="/{{Template.RoutePrefix}}/{{Template.Slug}}/create" save-return-url="/{{Template.RoutePrefix}}/{{Template.RoutePrevTemplateSlug}}/list"></wc-save-split-button>
      ...
      },
      {
        label: "Form",
        category: "group",
        description: "Form configured for HTMX",
      ...
      // --- Form Patterns ---
      {
        label: "Name Fields",
        category: "form",
        description: "First, M.I., Last in a row",
      ...
      },
      {
        ...
        category: "form",
        description: "Email + phone in a row",
     ...
     */
    _initSourceDoc(html) {
      const protected_ = html.replace(/<wc-select\b/gi, "<wc-select-safe").replace(/<\/wc-select>/gi, "</wc-select-safe>");
      const parser = new DOMParser();
    ...
    _safeSerialize(doc) {
      ...
      return html.replace(/<wc-select-safe\b/gi, "<wc-select").replace(/<\/wc-select-safe>/gi, "</wc-select>");
    }
    /**
    ...
    _getCleanHTMLFromSourceDoc() {
      if (!this._sourceDoc) return "";
      return this._sourceDoc.body.innerHTML.replace(/<wc-select-safe\b/gi, "<wc-select").replace(/<\/wc-select-safe>/gi, "</wc-select>");
    }
    /**
    ...
    _buildTreeFromSourceDoc(parent = null) {
      ...
      for (const child of root.children) {
        ...
        if (realTag === "wc-select") {
          const optionHTML = Array.from(child.children).filter((c) => c.tagName.toLowerCase() === "option").map((c) => c.outerHTML).join("\n");
    ...
    _buildDeviceOptions() {
      ...
      for (const d of groups.phone) {
        html += `<option value="${d.name}">${d.name} (${d.width}\xD7${d.height})</option>`;
      }
      html += `</optgroup>`;
    ...
    _wireEvents() {
      ...
      document.addEventListener("click", (e) => {
        if (settingsDialog && !settingsDialog.classList.contains("hidden") && !settingsDialog.contains(e.target) && !settingsBtn.contains(e.target)) {
      ...
      this.querySelectorAll(".ld-bp").forEach((bp) => {
        bp.addEventListener("click", () => {
          ...
          this.querySelector(".ld-device-select").value = "0";
    ...
    _applyDevice(device) {
      ...
      if (device.type === "responsive") {
        ...
        this._updateDeviceLabel(device.name, `${w}\xD7${h}`);
      }
      this._applyZoom();
    }
    _applyCustomSize() {
    ...
    _resolveAnyOf(prop) {
      ...
      for (const variant of prop.anyOf) {
        if (variant.format) return { ...prop, ...variant, anyOf: void 0 };
        if (variant.enum && variant.enum.length > 0 && !(variant.enum.length === 1 && variant.enum[0] === "")) {
          return { ...prop, ...variant, anyOf: void 0 };
        }
      }
      for (const variant of prop.anyOf) {
        if (variant.type) return { ...prop, ...variant, anyOf: void 0 };
      }
      return prop;
    }
...
function runDelete() {
  ctx.DB.DeleteByID(rdx.ConnName, rdx.DBName, rdx.AppData.Template.CollectionName, ctx.RecordID);
    ...
    _showPropertyPanel(type, properties, designerId) {
      ...
      fieldsEl.innerHTML = "";
      const nativeElements = /* @__PURE__ */ new Set(["button", "a", "h1", "h2", "h3", "h4", "h5", "h6", "p", "span", "label", "img", "hr"]);
      ...
      if (type === "wc-input") {
        ...
        commonProps.push({
          ...
          label: "Style",
          type: "select",
          value: properties["icon-style"] || "solid",
...
var WcSidebar = class extends WcBaseComponent {
  ...
  getDesignerHTML() {
    const container2 = this.querySelector(".wc-sidebar");
    if (!container2) return null;
    const children = container2.querySelectorAll(":scope > *");
    if (children.length === 0) return null;
    return Array.from(children).map((child) => {
      const tag = child.tagName.toLowerCase();
      const attrs = [];
...
if (!customElements.get("wc-sidenav")) {
  class WcSidenav extends WcBaseComponent {
    ...
    getDesignerHTML() {
      const container2 = this.querySelector(".wc-sidenav");
      if (!container2) return null;
      const children = container2.querySelectorAll(":scope > *:not(.closebtn)");
      if (children.length === 0) return null;
      return Array.from(children).map((child) => {
        const tag = child.tagName.toLowerCase();
        const attrs = [];
...
var WcTab = class extends WcBaseComponent {
  ...
  /**
   * Close all tabs except the one with the given label.
   * @param {string} label - The label of the tab to keep
   */
  closeOthers(label) {
    const tabNav = this.querySelector(":scope > .wc-tab > .tab-nav");
    if (!tabNav) return;
    const labels = Array.from(tabNav.querySelectorAll(":scope > button.tab-link")).map((btn) => btn.dataset.label).filter((l) => l !== label);
    labels.forEach((l) => this.removeTab(l));
  }
  /**
   * Close all tabs.
   */
  closeAll() {
    const tabNav = this.querySelector(":scope > .wc-tab > .tab-nav");
    if (!tabNav) return;
    const labels = Array.from(tabNav.querySelectorAll(":scope > button.tab-link")).map((btn) => btn.dataset.label);
    labels.forEach((l) => this.removeTab(l));
  }
  /**
   * Close all tabs to the right of the given label.
   * @param {string} label - The reference tab label
   */
  closeToRight(label) {
    const tabNav = this.querySelector(":scope > .wc-tab > .tab-nav");
    if (!tabNav) return;
    const buttons = Array.from(tabNav.querySelectorAll(":scope > button.tab-link"));
    const idx = buttons.findIndex((btn) => btn.dataset.label === label);
    if (idx === -1) return;
    const labels = buttons.slice(idx + 1).map((btn) => btn.dataset.label);
    labels.forEach((l) => this.removeTab(l));
  }
  /**
   * Close all tabs to the left of the given label.
   * @param {string} label - The reference tab label
   */
  closeToLeft(label) {
    const tabNav = this.querySelector(":scope > .wc-tab > .tab-nav");
    ...
    const idx = buttons.findIndex((btn) => btn.dataset.label === label);
    if (idx === -1) return;
    const labels = buttons.slice(0, idx).map((btn) => btn.dataset.label);
    labels.forEach((l) => this.removeTab(l));
  }
  _showTabContextMenu(event) {
    event.preventDefault();
  ...
  _render() {
    ...
    const tabNav = this.querySelector(":scope > .wc-tab > .tab-nav");
    if (tabNav && !tabNav._contextMenuWired) {
      tabNav._contextMenuWired = true;
      tabNav.addEventListener("contextmenu", this._showTabContextMenu.bind(this));
    }
    if (typeof htmx !== "undefined") {
      htmx.process(this);
    }
    ...
    async _loadKnowledgeBases() {
      ...
      for (const url of urlList) {
        try {
          ...
          const data = await response.json();
          const name = data?.meta?.name?.toLowerCase().replace(/\s+knowledge\s+base/i, "").replace(/\s+/g, "-") || url.split("/").pop().replace(".json", "");
...
${json}`);
    ...
    _matchKnowledgeByKeywords(message) {
      ...
      for (const mapping of keywordMap) {
        if (mapping.keywords.some((kw) => lower.includes(kw))) {
          if (!sections[mapping.kb]) sections[mapping.kb] = /* @__PURE__ */ new Set();
    ...
    _prepareMessages(userMessage) {
      ...
      if (this._isAssistantMode() && this._activeCommand) {
        ...
        const def = cmd.definition || cmd;
        const kbContext = this._buildAssistantContext(def);
        const cmdSystemPrompt = def.systemPrompt || systemPrompt;
        if (kbContext) {
          const maxContextChars = 8e3;
          const trimmedContext = kbContext.length > maxContextChars ? kbContext.substring(0, maxContextChars) + "\n\n[...truncated to fit context window]" : kbContext;
...
${fieldLines}`;
    ...
    _getWelcomeMessage() {
      ...
      if (this._isAssistantMode()) {
        ...
        const kbInfo = kbNames.length > 0 ? `I have knowledge loaded for: **${kbNames.join(", ")}**.` : "No knowledge bases loaded yet.";
...
var WcChartjs = class extends WcChart {
  static get is() {
    return "wc-chartjs";
  }
  static get observedAttributes() {
  ...
  _applyFetchedData(data) {
    ...
    if (data.showLegend !== void 0) {
      this.setAttribute("show-legend", String(data.showLegend));
    }
    if (data.showDataLabels !== void 0) {
...
// src/js/components/wc-busy-indicator.js
var WcBusyIndicator = class extends WcBaseComponent {
  static get is() {
  ...
  _createIndicator(type, size) {
    const container2 = document.createElement("div");
    container2.classList.add("busy-indicator-animation");
    const sizeMap = {
  ...
  _createDotsIndicator(dims) {
    const container2 = document.createElement("div");
    container2.classList.add("flex", "gap-2");
    for (let i = 0; i < 3; i++) {
  ...
  _createSkeletonIndicator(dims) {
    const container2 = document.createElement("div");
    container2.classList.add("flex", "flex-col", "gap-2", "w-full");
    container2.style.maxWidth = `${dims.width}px`;
    for (let i = 0; i < 3; i++) {
