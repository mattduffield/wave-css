...
`)+`
Assistant:`;c=await this._tokenizer(y)}let u=parseFloat(this.getAttribute("temperature")||"0.7"),p=parseInt(this.getAttribute("max-tokens")||"1000"),{TextStreamer:m}=d,h=new m(this._tokenizer,{skip_prompt:!0,skip_special_tokens:!0,callback_function:g=>{this._currentResponse+=g,this._updateMessage(this._currentMessageId,this._currentResponse)}}),b=await this._model.generate({...c,max_new_tokens:p,temperature:u,do_sample:u>0,streamer:h,return_dict_in_generate:!0});this.getAttribute("debug")==="true"&&console.log("[wc-hf-bot] Raw response:",this._currentResponse),this._emitEvent("bot:response-received",{botId:a,response:this._currentResponse})}catch(l){console.error("[wc-hf-bot] Failed to get response:",l),this._updateMessage(r,`Error: ${l.message}`),this._emitEvent("bot:error",{botId:a,error:l.message})}finally{this._isLoading=!1,this._sendButton.disabled=!1,this._input.focus()}}_prepareMessages(s){let r=[{role:"system",content:this.getAttribute("system-prompt")||"You are a helpful AI assistant. Be concise and friendly in your responses."}];return this._messages.slice(-10).forEach(c=>{c.role!=="system"&&r.push({role:c.role==="user"?"user":"assistant",content:c.content})}),r.push({role:"user",content:s}),r}_addMessage(s,a,r=!1){let l=Date.now().toString(),c={id:l,role:s,content:a,timestamp:new Date,isLoading:r};this._messages.push(c);let u=document.createElement("div");u.className=`wc-hf-bot-message wc-hf-bot-message--${s}`,u.dataset.messageId=l;let p=document.createElement("div");if(p.className="wc-hf-bot-message-bubble",r)p.classList.add("wc-hf-bot-message-bubble--loading"),p.innerHTML='<wc-loader size="90px" speed="1s" thickness="12px"></wc-loader>';else if(s==="bot"&&i&&i.marked){let m=i.marked.parse(a);p.innerHTML=m,this.getAttribute("debug")==="true"&&console.log("[wc-hf-bot] Parsed HTML:",m)}else p.textContent=a;return u.appendChild(p),this._messagesContainer.appendChild(u),this._messagesContainer.scrollTop=this._messagesContainer.scrollHeight,l}_updateMessage(s,a){let r=this._messagesContainer.querySelector(`[data-message-id="${s}"]`);if(r){let c=r.querySelector(".wc-hf-bot-message-bubble");if(c)if(c.classList.remove("wc-hf-bot-message-bubble--loading"),i&&i.marked){let u=i.marked.parse(a);c.innerHTML=u,this.getAttribute("debug")==="true"&&console.log("[wc-hf-bot] Updated HTML:",u)}else c.textContent=a}let l=this._messages.find(c=>c.id===s);l&&(l.content=a,l.isLoading=!1),this._messagesContainer.scrollTop=this._messagesContainer.scrollHeight}_updateStatus(s,a="info"){this._statusBar&&(this._statusBar.textContent=s,this._statusBar.className=`wc-hf-bot-status wc-hf-bot-status--${a}`,this._statusBar.style.display=s?"block":"none")}_adjustInputHeight(){this._input.style.height="auto",this._input.style.height=Math.min(this._input.scrollHeight,120)+"px"}_getWelcomeMessage(){return`Hello! I'm ${this.getAttribute("title")||"HF Assistant"} powered by Hugging Face. How can I help you today?`}_emitEvent(s,a){this.dispatchEvent(new CustomEvent(s,{detail:a,bubbles:!0,composed:!0}));let r=this.getAttribute("bot-id");r&&window.wc&&window.wc.EventHub&&window.wc.EventHub.broadcast(s,[`[bot-id="${r}"]`],a)}async _checkSystemCapabilities(){try{if(this.getAttribute("force-enable")==="true")return console.warn("[wc-hf-bot] Force-enabled, skipping capability checks"),!0;let s=localStorage.getItem("wc-hf-bot-success"),a=localStorage.getItem("wc-hf-bot-failure");if(a==="true"&&s!=="true")return this._unsupportedReason='AI models previously failed to load on this device. Use force-enable="true" to retry.',!1;let r="gpu"in navigator,l=document.createElement("canvas"),c=l.getContext("webgl2")||l.getContext("webgl"),u=!!c;if(!r&&!u)return this._unsupportedReason="WebGPU and WebGL are not available. A modern browser with GPU support is required.",!1;let p="unknown";if(c){let h=c.getExtension("WEBGL_debug_renderer_info");h&&(p=c.getParameter(h.UNMASKED_RENDERER_WEBGL))}let m=null;for(let h of o.KNOWN_CAPABLE_CONFIGS)if(h.gpu.test(p)){m=h;break}return this.getAttribute("debug")==="true"&&console.log("[wc-hf-bot] Hardware detection:",{renderer:p,detectedConfig:m,hasWebGPU:r,hasWebGL:u,previousSuccess:s,previousFailure:a}),m?m.capability==="none"?(this._unsupportedReason=`${m.name} is not capable of running AI models efficiently. This would cause your browser to freeze.`,!1):(m.capability==="low"&&console.warn("[wc-hf-bot] Low capability hardware detected. Performance may be poor."),this._detectedCapability=m.capability,!0):s==="true"?!0:(this._unsupportedReason=`Unable to verify GPU compatibility (${p}). To protect your browsing experience, AI models are disabled. Add force-enable="true" to try anyway.`,!1)}catch(s){return console.error("[wc-hf-bot] Error checking system capabilities:",s),this._unsupportedReason="Error detecting system capabilities. AI models are disabled for safety.",!1}}_renderUnsupportedUI(){this._isUnsupported=!0;let s=this.getAttribute("theme")||"bubble",a=this.getAttribute("title")||"HF Assistant";if(s==="bubble"){this.style.display="none",this._emitEvent("bot:unsupported",{botId:this.getAttribute("bot-id"),reason:this._unsupportedReason});return}this._container=document.createElement("div"),this._container.className="wc-hf-bot-container wc-hf-bot-container--unsupported",this._container.innerHTML=`
