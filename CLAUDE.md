...
**Button color variants** — compose with `.btn` (e.g. `class="btn btn-primary"`; also works with `.btn-sm`). Each variant just reassigns the shared `--button-*` tokens, so the base `.btn` structure/hover/active/focus/disabled is reused (no per-variant state selectors). Available: `.btn-primary`, `.btn-secondary`, `.btn-cancel`, `.btn-success`, `.btn-danger`, `.btn-warning`, `.btn-info`. **These were previously "phantom" classes** — used in templates (`wc-form-array`, `wc-chartjs`, …) but never defined in any CSS, so they rendered as a plain neutral `.btn`; now they're real. All verified WCAG AA (≥4.5:1 label vs fill) across all themes in both light & dark via `tests/button-variants-test.py`. Details: `primary`/`secondary`/`cancel` **track theme tokens**; `success`/`danger`/`warning`/`info` are **fixed semantic fills** deepened just enough that white text is AA in every theme (the raw `--*-color` accents are too light for white). `.btn-primary` deepens `--primary-bg-color` 60% toward black + white label (the theme's own `--primary-color` isn't reliably AA — `--primary-bg-color` spans bright-yellow→dark-coffee across themes); in `.crisp` it uses the lighter per-hue `--accent-fill-bg`/`--on-accent` via fallback. **`.btn-cancel`** is the dark/"opposite-of-primary" dismiss button (like the swal cancel): `--text-1` fill (black-ish in light themes, a light chip in dark) with a `--surface-1` label — always maximal contrast. (The swal cancel button itself still styles via `--secondary-bg-color` in `wc-prompt.js`; `.btn-secondary` matches that neutral look for regular buttons.) **Bare `.btn` auto-contrast (label legibility everywhere):** a plain filled `.btn` (no variant) previously inherited the theme default `--button-color` (a light swatch), which failed WCAG AA on the bright accent fill in **dark** themes (~1.9–2.6:1) and on dark earth-tone fills in **light** themes (~2.9–3.4:1). A zero-specificity `@supports`-guarded `:where(.btn:not(.btn-outline))` rule in `@layer wc.ui` now auto-picks **black or white** for `--button-color` **and** `--button-hover-color` from each button's own `--button-bg-color`/`--button-hover-bg-color` (same WCAG-luminance mechanism as wc-table's `--wc-on-primary`, crossover Y≈0.179). Because it's `:where()` (specificity 0), the seven semantic variants — which all set their own `--button-color`/`--button-hover-color` — still win untouched; `.btn-outline` is excluded (its label sits on the container, not a fill). **Channel clamp:** the luminance math wraps each relative-color channel in `clamp(0,<ch>,255)` because highly-saturated accents are out of the sRGB gamut and their `r/g/b` keywords can go negative → `pow(negative,2.4)=NaN` would collapse the pick to black on a dark fill; clamping gamut-limits the channels so the choice matches the rendered color (this same clamp was also applied to `--wc-on-primary`). Now **every filled button (bare `.btn` + all 7 variants) is AA (≥4.5:1) in resting AND hover across all 39 themes × light/dark** — verified in `tests/button-contrast-test.py` (1248 checks; bare `.btn` worst 4.65).
...
- `wc-select`: Dropdown select; `wcoptionsloaded` event fires after URL options load. **`mode="chip"`** multi-select; **`allow-dynamic`** adds raw typed values as chips; **`allow-custom`** (taggable) adds a typed value not in the list as a chip but **normalized** (trim + collapse whitespace + case via `normalize="title|lower|upper|none"`, default `title`) and **deduped case-insensitively** (`eggs`/`Eggs` collapse; a typed value matching an existing option case-insensitively reuses that option's canonical value). Pairs with `wc-form-array` `control="wc-select"` for a list+custom multi-chip column. The chip text input suppresses the browser's native autofill/history popup (`autocomplete=off` + autocorrect/autocapitalize/spellcheck off + `data-lpignore`/`data-1p-ignore`); the options dropdown is a solid theme surface (`--component-bg-color`/`--text-1`) with a shadow at `z-index:1000` (readable in light/dark, above sibling UI). **Chip pre-fill matches case-insensitively** on the option value (so `"Epipen"`→`EpiPen` option, canonical); a value matching no option becomes a **custom chip** under `allow-custom` (legacy free-text like `"Guanfacin"` renders instead of vanishing)
...
- `wc-record-lookup`: **Typeahead that searches an EXISTING collection** via a server endpoint and, on select, **emits the record or fills sibling fields** — powers "match-at-entry" hints, "use this family", and generic attach-existing flows. **Server is the source of truth** (tenant-scoped, display-safe fields); the component only displays what the endpoint returns and **never blocks typing**. Attributes: `endpoint` (GET; appends `?q=<query>` + every `param-*` attr as a query param), `min-chars` (default 2), `debounce` (default 300ms), `label-field` (display, default `label`; dotted paths + literal-key-first), `value-field` (default `id`), `mode`, `fill-map` (JSON, `mode=fill`), `keep-text` (`mode=select`), `placeholder`, `lbl-label`, `disabled`, `name`. **`mode="select"`** → emit `record:selected` with the full record, clear the input unless `keep-text`. **`mode="fill"`** → for each `fill-map` `{target_field_name:"result.path"}` entry, resolve the dotted path and set the sibling field found by `name` (scoped to the closest `<form>`, else document) via property + `value` attr + inner control, dispatching `input`+`change` (works for `wc-input`/plain input/select/textarea) — **then also emits `record:selected`** (e.g. fill parents, then trigger a children load). **`mode="hint"`** → non-blocking inline banner "Looks like **{label}** — use existing?" with a primary action button that emits `record:selected` + a × dismiss (no dropdown). Dropdown reuses the **`wc-address` look + keyboard nav** (Arrow/Enter/Escape/click, mousedown-before-blur); suggestion text is colored **explicitly** with `--text-1` (don't rely on inherited color). The search-icon offset (`.rl-input` `padding-left`) is emitted **unlayered** so it beats the `@layer wc.ui` bare `input { padding }` rule (which is a later layer than `wc.usage` and would otherwise reset padding-left and push the placeholder under the icon). Empty results clear the dropdown/hint. Events (element + document, lowercase canonical + legacy colon alias): `wcrecordselected`/`record:selected` `{record}`, `wcrecordsearch`/`record:search` `{q}`. Optional `el.searchHandler = async (q, params)=>results[]` routes the search through your own client (demo/tests) — still server-authoritative. Extends `WcBaseComponent` (NOT form-associated — it emits/fills, doesn't submit its own value). `display:contents`; `@layer wc.usage`. Verified in `tests/record-lookup-test.py`
...
- `wc-form-array`: Declarative repeatable sub-form for an **array-of-objects**, lives inside `<wc-form>`. Extends `WcBaseComponent` (NOT form-associated — its `name` is only a dotted-index prefix). Renders native form controls named `${name}.${index}.${field}` (e.g. `line_items.0.product_id`) so the server's dotted-index save path reconstructs the array — no JSON serialization, no custom endpoint. Columns declared via `wc-form-array-column` children; `value` is a JSON array of row objects. **Hard guarantee: after any add/remove, every control name is renumbered so indices stay contiguous `0..n-1`** (gaps create null/empty array holes). `min-rows`/`max-rows` enforced; empty rows are excluded on submit (native `submit` capture + `htmx:configRequest`) so a blank trailing row never serializes a junk object. `readonly` renders non-editable static text (select shows the option label). **`layout`**: `table` (default, one row/item + shared header) or **`card`** — each item is a bordered card (`.card`/surface look) with columns in a **responsive label-above grid** (`repeat(auto-fit, minmax(12rem,1fr))`, wraps 1→2→3), readable for many-field sub-forms (e.g. a 10-field guardian). Card mode is purely visual: each item is still a `.wc-form-array-row` with the same `${name}.${index}.${field}` controls, so naming/reindex/blank-row/`value`/events are identical. **`item-title="Parent / Guardian {index1}"`** labels each card header — tokens `{index}` (0-based), `{index1}` (1-based), `{field}` (live column value); refreshes on add/remove/reindex/input. **Programmatic row API** (so a `wc-record-lookup` can drive it): `addRow(data?)` (append pre-filled from `{field:value}`), `setRow(index, data)` (fill/replace at index; partial merges), `removeRow(rowOrIndex)`; declarative hook — dispatch a **`wc-form-array:populate`** (or `wcformarraypopulate`) CustomEvent `{rows:[…]}` on the element to append rows ("use this family"). htmx-swap-safe; styles in `@layer wc.usage`. Event: `wcformarraychange` (detail `{ name, rows }`; legacy alias `wc-form-array:change`). Public API: `addRow(data?)`, `setRow(index, data)`, `removeRow(rowOrIndex)`, `rows` get/set, `value` get/set
- `wc-form-array-column`: Configuration-only child of `wc-form-array` (renders nothing, never submits). Attributes: `field` (required — the object key / dotted `sub`), `label`, `type` (text/number/date/select/**textarea**/**tel**/**address**), `options` (JSON array — inline `{key,value}` or a collection of records for a reference/FK column), `option-value`/`option-label` (which members are stored value vs visible label), `min`/`max`/`step`/`placeholder`/`required`, `col-class` (width/align). **New column types/options:** `type="textarea"` (+ `rows`, default 2) for multi-line fields; **`full-width`** makes the column span the whole card on its own row (card layout, `.wc-fa-field` → `grid-column:1/-1`) — ideal for a long textarea/medical field; **`mask`** applies a WcMaskHub mask (`phone`|`ssn`|`zip`|`zipPlus4`|`date`|`currency`) via `wc.MaskHub.applyMaskToElement` (on `wcready` if the hub isn't ready — formats pre-filled AND newly-added rows), and **`type="tel"` implies `mask="phone"`** so a phone column formats exactly like `wc-input type=tel` (**cold-load safe:** masking defers until `MaskHub.maskConfigs` exists / the `wcready` event, wrapped in try/catch, so a not-yet-initialized hub on a full page load can't throw and blank the array; each control is also created via a try/catch fallback to a plain input); **`type="address"`** renders a per-row `wc-address` (geocode autocomplete; `geocode-url` default `/api/geocode`, `countries` passthrough) whose **selection fills the row's address sub-fields** under `${name}.${index}.${field}.{street,city,state,postal_code,county,country,lat,lng,formatted_address}` (visible input = `street`; rest hidden by default, populated on `wcaddresschange`) → server reconstructs a **nested** address object. **`show-fields`** renders some parts as **visible, editable** inputs beneath the street (boolean → City/State/Zip; or a comma list) — still `${field}.{sub}`-named + auto-filled on geocode, but staff can see/correct them; absent → hidden-only (backward-compat). Each control carries a dotted `data-col` so reindex/collect/blank-row stay unchanged (**reindex-safe**, one value per sub-field, no double-submit); `rows`/change-event nest the dotted keys back to `{field:{street,city,…}}`. Works in card **and** table layouts (full-width row-spanning is card-only). **`icon="envelope|phone|qrcode|…"`** renders a leading decorative `wc-fa-icon` inside the input (left-aligned, input padded — same look as `wc-input`'s type icons; padding emitted **unlayered** to beat the `wc.ui` bare `input{padding}`); **defaults by type** (`tel`→phone, `email`→envelope; explicit `icon` wins), and for `type="address"` it becomes the inner `wc-address`'s `icon-name` (e.g. `location-dot`). The icon is **decorative** — no `data-col`/name, excluded from reindex/naming, so it never becomes a `?` glyph. **`required`** renders a visible `*` on the column label (card `.wc-fa-field` + table `.wc-form-array-hcell`, matching `wc-input`). **Generic control passthrough:** instead of the fixed type list, **`control="<tag>"`** renders **any** Wave form component and **passes through every non-reserved attribute** (reserved: `field, label, type, control, full-width, col-class, required, icon`; `options`/`option-value`/`option-label` become `<option>` children). So a column can be `control="wc-select" multiple mode="chip" allow-custom` (list + custom chips), `control="wc-combobox" url="…" display-member value-member` (server search), or any future component — no new column type. **Array-valued columns:** a control with an inner `<select multiple>` makes the column value an **array** — pre-fill from a `value` array (→ `<option selected>` children), the `rows`/change-event return an array, and it submits as multiple `${name}.${index}.${field}` entries → server array of strings; reindex + blank-row still work (host + inner select renumbered; blank rows disable inner controls too). Scalar controls unchanged. One `wc-form-array-column` per schema `def` field — generation-friendly
...
- `wc-rich-text`: Form-associated rich-text / markdown EDITOR (the editable counterpart to `wc-markdown-viewer`). Lives in `<wc-form>` and submits its content as a normal named value (FACE — `setFormValue` under the host `name`), so the standard save path stores it like a `wc-textarea`. `mode="markdown"` (default, submits Markdown) or `"html"` (submits DOMPurify-sanitized HTML). `value` seeds + round-trips. `toolbar` = `basic`/`full`/comma-list (bold,italic,**underline**,h2,h3,ul,ol,link,quote,code,image,table) via `wc-fa-icon`. `required` integrates with form validity (`setValidity`/valueMissing). Lazily loads marked/turndown/DOMPurify from cdnjs (like `wc-code-mirror`); sets `_deferReady` → **await `.ready`** before reading/setting `value`. Sanitizes on input + paste in both modes. Markdown-mode preview toggle renders into a real `wc-markdown-viewer`. htmx-safe; styles in `@layer wc.usage`. Event: `wcrichtextchange` `{name, value, mode}` (legacy `wc-rich-text:change`). **Device image upload:** set `image-upload-url` (+ optional `csrf`) → the toolbar image button opens a device file picker (`accept=image/*`), POSTs the file as multipart field `image` (+ `csrf_token` field when `csrf` set) same-origin, expects JSON `{url}`, and inserts the returned **hosted** url at the caret (DOMPurify-sanitized; caret saved before the dialog + restored; brief "Inserting image…" chip; fail-safe alert + no insert on error). **Absent → legacy Image-URL `prompt()` (no regression).** Email clients need hosted images, not data URIs. Default to `markdown` storage for long-text/richtext schema fields. Verified in `tests/rich-text-image-test.py`
...
- `wc-barcode`: Scannable code (QR default, `symbology="code128"`) + human-readable value, rendered as **crisp print-quality SVG** with **bundled, offline** generators (no CDN — a compact QR encoder + Code128). The code is **always pure black-on-white** (never theme-tinted, so scanners read it); caption/label use `--text-1`. Attributes: `value` (required; empty → renders nothing), `symbology` (`qr`|`code128`), `label` (`right` default|`bottom`|`none`), `size` (QR px default 96 / Code128 bar-height px default 64), `ec-level` (`L`|`M` default|`Q`|`H`), `caption` (small text above). Display-only: `value` get/set property re-renders, no form/events; `role="img"` + `aria-label="barcode <value>"`. `@media print` forces black-on-white / no shadow-margins for a clean label. QR verified scannable by decoding it back (jsQR) in `tests/barcode-test.py`
- `wc-checkin`: Touch-friendly **scan-to-check-in / check-out station** (staffed tablet). Captures a scanned code (**USB keyboard-wedge** and/or **camera**), POSTs to `endpoint`, and renders the server's **allow/deny** verdict in big glanceable states. **THE SERVER IS AUTHORITATIVE** — the component never decides allow/deny, never caches/infers authorization, and never receives the authorized barcodes (only display **names** + a yes/no). Attributes: `mode` (`in` default | `out`), `scan-source` (`usb` default | `camera` | `both`), `endpoint`, `event-id`, `session-id`, `csrf` (sent as `X-CSRF-Token` header + `csrf_token` in body), `allow-override` (show admin Override on a DENIED check-out only when response has `override_allowed`), `beep` (default true; WebAudio tone, no asset). **Request** `{ mode, scanned_barcode, event_id, session_id, step, attendee_id?, csrf_token?, override? }` — `step` is `identify` on the first scan, `authorize` on the pickup scan. **Response** (rendered as-is) `{ allowed, reason, attendee:{id,name,photo?}, authorized_names?, requires_pickup_scan?, recorded_id?, override_allowed? }`. **Check-IN** = one scan → ALLOWED/DENIED + reason + attendee card. **Check-OUT** = two-step: first scan identifies the child (`requires_pickup_scan:true`) → shows child card + `authorized_names` (names only) + "Scan an authorized pickup"; the pickup scan posts `step:authorize` + `attendee_id` → final verdict (+ "Released" on allow); Override re-posts with `override:true`. **USB wedge:** buffers rapid keystrokes ending in Enter; inter-keystroke timing (>60ms gap resets buffer) tells a scanner burst from human typing (min length 3). **Camera:** native `BarcodeDetector` (QR + 1-D, zero load) where available; browsers without it (Safari/Firefox) **fall back to a lazily-loaded ZXing decoder** (via Wave's shared loader → WaveAssetBase self-host + CDN fallback; fetched only the first time a fallback browser opens the camera) so the camera works everywhere; only if neither loads → "use a USB scanner" message. **Fail-safe:** any network/parse error → ERROR state, **never ALLOWED**. Events (element + document, lowercase canonical + legacy colon alias): `wccheckinscanned`/`checkin:scanned` `{code,source,step}`, `wccheckinresult`/`checkin:result` (full response), `wccheckinerror`/`checkin:error` `{message}`. Optional `el.requestHandler = async (payload)=>response` routes the POST through your own client (demo/tests) — still server-authoritative. `display:contents`; `@layer wc.usage`. Verified in `tests/checkin-test.py`
...
- `wc-tab`: Tab container (supports right-click context menu on removable tabs with Close, Close Others, Close All, Close to Right, Close to Left; public methods: `closeOthers(label)`, `closeAll()`, `closeToRight(label)`, `closeToLeft(label)`; uses wc-context-menu internally; `no-hash` attribute suppresses URL hash updates on tab click). Active tab is shown by an **accent indicator** (inset underline, `--wc-tab-indicator-color` → `--accent`/`--primary-bg-color`) + bolder label — legible regardless of background contrast (`.crisp` light/dark, or a strip on a `.card`); hover is a separate subtle highlight. Theme-tunable tokens (with fallbacks): `--wc-tab-active-bg`, `--wc-tab-hover-bg`, `--wc-tab-body-bg`, `--wc-tab-indicator-color`, `--wc-tab-active-color`, and nested depth `--wc-tab-body-bg-1`/`-2` (default `--surface-3`/`--surface-4` — nesting steps through surfaces, not `#000`-mix, so it works in light themes). **Fill:** horizontal (default) tabs are a full-height flex column (`wc-tab:not([vertical]) .wc-tab` → `display:flex;flex-direction:column;flex:1 1 0%;min-height:0;height:100%`) and the active panel grows (`.wc-tab-item.active` → `flex:1 1 0%;min-height:0`), so the `.tab-body`(flex:1)→active-panel→content chain fills the parent — a tab fills its column regardless of content height (short = no gap, tall content with an inner `overflow:auto` scroller scrolls internally). `height:100%` is inert without a definite-height parent, so tabs in normal flow are unaffected; the vertical variant is untouched
...
### Data Components
- `wc-tabulator`: Advanced data table; supports `auto-columns` attribute for auto-generating columns from data, `data` attribute for inline JSON arrays. **Header (column show/hide) menu:** `header-menu-columns="N"` lays the column-toggle list out in N side-by-side columns (default 1 = single vertical list; Hide/Show Filter + separator stay full-width, the checkboxes flow into an N-column grid). **Header filters:** auto-columns get **no** `headerFilter` by default (so no filter row shows — declarative columns use `header-filter=…`); add **`auto-header-filter`** to give each scalar auto-column a text filter (object/array columns skipped). The menu's "Show Filter" now regenerates auto-columns rather than calling `getColumnsConfig()` (which returns `[]` for auto-columns and would blank the table). **Auto-columns are FLAT + heterogeneous-safe:** one column per top-level key (nested objects/arrays are NOT exploded into `parent.child` columns — they render as one cell with compact JSON / `[N items]`, scalars as-is); a field that is an object in some rows and a scalar in others yields a single `object` column and never crashes (auto-columns don't force `nestedFieldSeparator='.'`, so Tabulator never traverses/mutates a nested path). `_id` stays frozen-first with title-cased headers; declarative `<wc-tabulator-column>` columns can still opt into nested fields. **Dynamic server-side (remote) pagination:** public method `setRemoteData(url)` switches the table to a new remote query at runtime — sets `ajaxURL`, flips `paginationMode`/`filterMode`/`sortMode` to `'remote'`, clears inline data, and reloads from page 1 (`this.table.setData(url)`); works even after inline `data` was set (flips local→remote). `ajax-url` is an observed attribute — setting it non-empty calls `setRemoteData` (empty/removed = no-op). The endpoint returns `{data, last_page, last_row}` and the built-in First/Prev/Next/Last controls fetch each page. With `auto-columns` (and no declarative `<wc-tabulator-column>` children), remote paging builds the columns from the **first page** of the ajax response (in `handleAjaxResponse`, guarded so it runs once per source; `setRemoteData` resets the guard so a new query regenerates columns for the new shape). Counterpart `setLocalData(data)` reverses the switch — flips `paginationMode`/`sortMode`/`filterMode` back to `'local'`, clears the ajax URL, and loads the supplied in-memory dataset (a JSON string or array, same shapes as the `data` setter) paged locally. **Per-response transform:** `ajax-response-transform="fnName"` (resolved via the `wc-tabulator-func` funcs mechanism, then class/global/inline) passes `response.data` (the rows array) through `(rows) => rows` before render, preserving `last_page`/`last_row` — use it to convert Extended-JSON to plain values. Existing inline-`data` local-pagination tables are unaffected
...
Two components for a **multi-zone sound/vision board**: `wc-fx-board` plays audio/video **locally on a station device**; `wc-fx-console` is the **master** that remote-controls zones over a host-provided WebSocket. Resources are **typed** — `audio`/`video` now, `light` **reserved** (rendered as a labeled, disabled "coming soon" pad so the type is forward-compatible). Shared quality: Wave semantic tokens only (no hardcoded hex; light+dark), active/playing states use `--primary-bg-color` + the `--wc-on-primary` auto-contrast token (extended to both FX hosts), real `<button>` pads with `aria-pressed`/`aria-label` (incl. shortcut)/`focus-visible`, and a playing pulse gated behind `prefers-reduced-motion`. CSS is scoped in `src/css/main.css` (`@layer wc.usage`). No third-party deps. Verified in `tests/fx-board-test.py` (30 checks) and demoed in `views/fx-board.html`.
  - **RESOURCE shape** (JSON): `{ id, type:"audio"|"video"|"light", name, category, file_url, shortcut_key, volume(0..1), loop, is_background, order }`.
  - **WebSocket protocol** — COMMAND (master→zone): `{ type:"cmd", zone, action, resource_id?, value? }`, `action ∈ "play"|"stop"|"stopAll"|"volume"|"background"|"requestState"`. STATE (zone→masters): `{ type:"state", zone, playing:[{id,type,is_background}], master_volume }` (`master_volume` is a **0..1 fraction**; a zone's routing key is its `event_name`, falling back to `id`). Both components accept `ws-url` to open/drive the socket AND expose **`.send(obj)`** (sends over the socket if open AND dispatches a `message` CustomEvent so a host can forward it) + **`.receive(json)`** (feed an incoming message; object or JSON string) — so a host can wire its own transport.
- `wc-fx-board`: Per-station player. Attributes: `resources`(JSON), `categories`(JSON `[{name,icon,color}]`), `master-volume`(0..100, def 80), `show-shortcuts`(bool → number keys 1..9 on the first pads, or a resource's `shortcut_key`), `zone`(string), `controllable`(bool), `columns`(int), `ws-url`. Category-grouped pads; **audio** via `Audio()` (effects **overlap/retrigger**; a fresh element per trigger), **video** into a single inline `<video>` surface. **Background is EXCLUSIVE** (a new background stops the prior; honors `loop`) and a background pad toggles off when re-clicked; "Stop All" stops everything. **Effective volume = master(0..1) × resource.volume**; the master slider **live-adjusts** playing media. Local controls always work; when **`controllable`** it ALSO executes remote COMMANDs addressed to its `zone` and, after any change, **emits STATE** (via `.send`/socket + `message` event). `type:"light"` pads are disabled no-ops. Lazy media (`preload` on demand), **fail-soft** on a bad `file_url` (pad → `is-error`), reflects `resources` changes (HTMX swap), clean disconnect (stops media, closes socket, removes listeners). Public API also: `.play(id)`/`.stop(id)`/`.stopAll()`/`.setMasterVolume(0..100)`. Events (bubbling, composed, lowercase canonical + legacy colon alias): `wcfxboardplay`/`wc-fx-board:play` + `wcfxboardstop`/`wc-fx-board:stop`, detail `{id,type,zone}`; plus a `message` event (detail = outgoing protocol object). **NB** construct declaratively (HTML/parser or `insertAdjacentHTML`), not via `document.createElement` (the wrapper is appended in the constructor, matching the other Wave components).
- `wc-fx-console`: Master console — **NO local playback, ever**. Attributes: `zones`(JSON `[{id,name,event_name,resources:[…],categories?}]`), `master-volume`, `ws-url`. Zone selector (tab/list); for the SELECTED zone the same grouped pads + master volume + Stop-All, but a pad **SENDS a COMMAND** to that zone (never plays). On zone select it sends `action:"requestState"` (and on socket open it requests state for every zone). Shows **LIVE STATE** from incoming STATE messages: a per-zone "playing" count indicator in the zone list (whole floor at a glance) + a "now playing" chip list and pad highlights for the selected zone. Reflects `zones` changes. Public API also: `.selectZone(idOrKey)`. Events: `wcfxconsolecommand`/`wc-fx-console:command`, detail `{zone,action,resource_id,value}`; plus a `message` event (detail = outgoing protocol object).
...
### Data Components
- `wc-table`: Lightweight data-driven table (url, items, sorting, formatting, events). **Grid features (client-side, work in BOTH modes):** `paginate` + `page-size="N"` (default 10) → pager (« Prev · numbered + `…` ellipsis · Next » · a **Go to page** number input · an **X–Y of N** summary; public `_goToPage(n)` clamps 1..totalPages). `searchable` (+ `search-placeholder`) → debounced (~200ms) case-insensitive search box; scope to specific columns with `<wc-table-col … filterable>` (if any is filterable, search matches only those). `row-numbers` → leading `#` column numbered **globally across pages** (`page*pageSize + idx + 1`). Events: `wctablepage`/`wc-table:page` `{page,totalPages}`, `wctablefilter`/`wc-table:filter` `{query}` (+ existing `wctablesort`/`table:sort`). **TWO MODES:** (1) **data mode** — `<wc-table url|items>` renders cells from data (unchanged). (2) **enhance mode** — **auto-detected** when a `<wc-table>` wraps an authored `<table>` and has **no `url`/`items`**: it enhances the authored `<tbody> > <tr>` rows **in place** (reorder / show-hide / prepend a number cell) so **composite cell markup is preserved exactly** (avatar+name+DOB, multi-line, badges). Enhance search matches `tr.dataset.search` (author lowercase blob) else `tr.textContent`; enhance sort: `<th class="wc-sortable">`(or `data-sortable`) click sorts by that column's `td.dataset.sort` (else `td.textContent`), numeric when `th[data-type="num"|"date"]` else `localeCompare`, toggles asc/desc; paginate detaches non-page rows from tbody (keeps `nth-child` striping correct) and retains them in the model; row-numbers reuses an authored leading `<th>#</th>` if present else injects one. **External search/filter binding (both modes):** `search-input="<css-selector>"` binds an **external** `<input>` (via `document.querySelector`) as the search source instead of rendering the internal box — same debounce + `wc-table:filter` pipeline, **re-resolved on every (re)connect** so it survives an htmx fragment swap that replaces the `<wc-table>` (the external input lives outside the swapped region). Put the search box in an app-owned filter bar (e.g. beside a Master Event `<select>`); public `setSearch(query)` drives it from JS. **`filter-attr="data-event"`** is a second filter axis: a row passes when the named attribute's whitespace-token list includes the active value (enhance reads `tr[filter-attr]`; data mode reads the field with leading `data-` stripped) — set the value via `filter-input="<selector>"` (binds an external `<select>`), the `filter-value` attr, or public `setFilter(value)`; search AND attr-filter combine. **Stable column widths (enhance+paginate):** paged tables default to `table-layout:auto`, so each page's differing content made the headers jump on Prev/Next — now, **automatically** when enhance + `paginate` (opt out `layout="auto"`/`fixed-columns="false"`; opt in without paginate via `fixed-columns`/`layout="fixed"`), the component measures each column's natural width over the **full matching set**, pins them via a generated `<colgroup>`, and switches to `table-layout:fixed` so paging/sorting only swap row content. Recomputed on init / filter-change / container resize (debounced `ResizeObserver`), **not** on page change. Author overrides skip measuring: `column-widths="120px,,30%"` (aligned to header cells incl. `#`) or a per-`<th>` width; `.wc-rownum` stays narrow. A **`display:none` column** (responsive `hidden lg:table-cell`-style `<th>`/`<td>`) is **skipped when building the `<colgroup>`** (no `<col>` slot) so remaining columns stay aligned instead of shifting/overlapping; the ResizeObserver rebuilds it when a breakpoint toggles visibility. **Pager layout is CSS grid** (`auto 1fr auto auto auto` → prev·pages·next·jump·summary) so `« Prev »`/`Next »`/jump/summary sit at **fixed x-positions on every page** (park the cursor on Next and keep clicking) — page numbers re-center in the `1fr` track without displacing anything; buttons use `min-width`+`tabular-nums` and the `X–Y of N` summary is figure-space-padded to a constant width; wraps to two rows under 640px. **URL state sync (opt-in `sync-url`):** page/search/filter-value/sort are mirrored into the query string so reload / bookmark / share / Back-after-row-drilldown return to the same view. Writes on change (default `history.replaceState`; `url-history="push"` → `pushState` so Back/Forward step pages); **clears a param at its default** (search cleared → drop `q`; page→1 → drop `page`; filter→all → drop `filter`; sort→none → drop `sort`) so pristine = clean URL; only this table's params are touched (others preserved). **Namespace** via `url-key="participants"` → `participants_page`/`_q`/`_filter`/`_sort` (e.g. `_sort=name:asc`); bare `page`/`q`/`filter`/`sort` without a key. **Restores** on connect (htmx-swap safe) and reflects the restored search/filter back into the bound external `search-input`/`filter-input` (two-way); `popstate` re-applies for Back/Forward. Public: `goToPage(n)`, `setSearch(query)`, `setFilter(value)`. **Header contrast:** header/`#`-column/primary-cell text uses the auto-contrast token **`--wc-on-primary`** (on `wc-table`/`.wc-table`) which picks **black or white** by the WCAG luminance of `--primary-bg-color` (relative color + `pow()`, crossover Y≈0.179) — so it's AA-legible on the accent in **every theme, light + dark** (the static `--primary-color` flipped light in dark themes → ~1.9:1; now ≥4.5:1, verified 10 themes × light/dark). Falls back to `--primary-color` where relative color/`pow()` aren't supported (`@supports`). **Pager button contrast:** every pager button (« Prev / page numbers / Next », active + inactive) is accent-filled and routes its text through the **same `--wc-on-primary`** token (via `.wc-table-pager .btn { --button-bg-color: --primary-bg-color; --button-color: --wc-on-primary; … }`), because the generic `.btn` `--button-color` flip-flops per theme and failed AA on the bright accent (dark themes, ~1.9–2.6:1) and on dark earth-tone accents (light themes, ~2.9–3.4:1); now ≥4.5:1 (5–13:1) across all 39 themes × light/dark. **HTMX:** after every render (initial + pagination/sort/filter) the freshly-built DOM is handed back to HTMX (`htmx.process` on the render region + pager in data mode; the authored table + pager chrome in enhance mode), so `hx-*` on cell content (drill-in `<a hx-get hx-target hx-indicator>`) does an HTMX swap with the busy indicator instead of a full navigation — `innerHTML`/re-attach alone bypasses HTMX. **Scale:** enhance mode renders all rows up front (fine for hundreds); for 10k+ use data mode with server paging. Verified in `tests/table-features-test.py` + `tests/table-htmx-test.py`
- `wc-table-col`: Column definition for wc-table (field, label, sortable, **filterable**, align, format). **Rich cells:** `type="html"` (trusted innerHTML — caller owns escaping) or `formatter="badge|link|datetime"` (built-ins escape their own text → XSS-safe). `badge` → `.badge .badge-{variant}` via `formatter-map` (`{value:variant}` JSON, default `muted`); `link` → `<a href>` from `formatter-href` token template (e.g. `/x/order/{_id}`, tokens URL-encoded, htmx-boost friendly); `datetime` → `formatter-format` luxon-preset name (`DATE_MED`/`DATETIME_MED`/…) via Intl, consistent with `wc-tabulator`. `formatter` wins over `type="html"` (+ warning). **Sorting uses the raw field value**, not the rendered HTML; columns with neither attr are unchanged (plain escaped text). **`formatter="run-status"`** — live SSE cell: an active/running row shows a spinner + step text streaming from `formatter-events-url` (`{token}` template, URL-encoded), a resting row renders identical to `badge`. `formatter-active-field` (row field = running), `formatter-event-name` (NAMED SSE event — default `message`; the run stream emits `step_change`), `formatter-live-path` (dotted path w/ negative indices, e.g. `stack.-1.name`) or `formatter-live-field` (flat key, default `status`; text only), `formatter-done-when` (`prop` truthy or `prop=value`, e.g. `event=end`). With `formatter-event-name="step_change"`, live-path defaults to `stack.-1.name` and done-when to `event=end` (run-state defaults); terminal is payload-driven (socket may stay open with heartbeats after `end`). The component **owns the EventSource lifecycle** — reconciled by run id across re-renders (rebind/close/no-reopen), all closed on items/url re-set, row removal, or disconnect (no leaks). Emits one `wcrunstatuscomplete` (`wc-run-status:complete`) `{runId, row}` per run at terminal; the host then refreshes the authoritative verdict
...
- `views/checkin.html` - wc-checkin demos: check-IN (usb + camera) with ALLOWED/DENIED, and two-step check-OUT with authorized-pickups prompt + Override; in-page `requestHandler` mock (server-authoritative) + event log; Simulate-scan boxes stand in for a physical scanner
- `views/record-lookup.html` - wc-record-lookup demos: `mode="select"` (emit record), `mode="hint"` (use-existing banner), and `mode="fill"` ("use this family" filling dotted-name sibling fields); in-page `searchHandler` mock + event log
