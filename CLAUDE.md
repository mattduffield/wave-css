...
- `wc-form-array-column`: Configuration-only child of `wc-form-array` (renders nothing, never submits). Attributes: `field` (required — the object key / dotted `sub`), `label`, `type` (text/number/date/select/**textarea**/**tel**/**address**), `options` (JSON array — inline `{key,value}` or a collection of records for a reference/FK column), `option-value`/`option-label` (which members are stored value vs visible label), `min`/`max`/`step`/`placeholder`/`required`, `col-class` (width/align). **New column types/options:** `type="textarea"` (+ `rows`, default 3) for multi-line fields; **`full-width`** makes the column span the whole card on its own row (card layout, `.wc-fa-field` → `grid-column:1/-1`) — ideal for a long textarea/medical field; **`mask`** applies a WcMaskHub mask (`phone`|`ssn`|`zip`|`zipPlus4`|`date`|`currency`) via `wc.MaskHub.applyMaskToElement` (on `wcready` if the hub isn't ready yet), and **`type="tel"` implies `mask="phone"`** so a phone column formats exactly like `wc-input type=tel`; **`type="address"`** renders a per-row `wc-address` (geocode autocomplete via `geocode-url`, default `/api/geocode`) built via the parser and **reindex-safe** (the custom element's `name` + its inner named input are renumbered on add/remove — single submitted value, no double-submit). Works in card **and** table layouts (full-width row-spanning is card-only). One `wc-form-array-column` per schema `def` field — generation-friendly
