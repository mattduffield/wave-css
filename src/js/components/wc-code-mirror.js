...
if (!customElements.get('wc-code-mirror')) {
  class WcCodeMirror extends WcBaseComponent {
    ...
    async _createInnerElement() {
      ...
      // Wire context menu listener now that editor exists
    ...
    async renderEditor(initialValue) {
      ...
      // Same shape for link-includes — attribute or pre-queued flag.
    ...
    _paintStepBands() {
      ...
      // despite being in the DOM. Reading defaultTextHeight() at marker
      // creation time keeps the band in sync with the active theme/font.
    ...
    _installLinkIncludesListeners() {
      ...
      this._linkIncludesMouseMove = function (e) {
        ...
        const pos = editor.coordsChar({ left: e.clientX, top: e.clientY });
        if (!pos) { wrapper.classList.remove('cm-link-includes-active'); return; }
        const line = editor.getLine(pos.line);
