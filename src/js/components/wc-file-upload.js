...
class WcFileUpload extends WcBaseFormComponent {
  ...
  _buildSkeleton() {
    ...
    // Build the whole skeleton via the parser path (innerHTML). The child custom elements
    // (wc-fa-icon, wc-progress) append their own DOM in their constructors, which is legal
    // when the parser upgrades them but throws NotSupportedError via document.createElement()
    // ("The result must not have children") — so we must NOT createElement() them.
    // NB: the file <input> intentionally has NO `name` — the raw file is never submitted;
    // only the FACE url value is.
    this.componentElement.innerHTML = `
      ...
      <div class="wc-file-upload-dropzone${disabled ? ' is-disabled' : ''}" role="button" tabindex="0">
        ...
        <div class="wc-file-upload-hint">${accept ? this._esc(accept) : 'Any file'}${maxSize ? ` · up to ${this._esc(maxSize)}MB` : ''}</div>
  ...
  _renderPreviews() {
    ...
    this._files.forEach((f, idx) => {
      ...
      // Build the item via the parser path (innerHTML on a plain <div>): the non-image branch
      // needs <wc-fa-icon>, whose constructor appends children — legal for the parser but a
      // NotSupportedError via document.createElement().
      const media = isImage
        ? `<img class="wc-file-upload-thumb" src="${this._esc(f.url)}" alt="${this._esc(f.name)}" loading="lazy">`
        : `<wc-fa-icon name="file" size="1rem" class="wc-file-upload-fileicon"></wc-fa-icon>`;
      const sizeHtml = f.size != null
        ? `<span class="wc-file-upload-itemsize badge badge-muted">${this._esc(this._formatSize(f.size))}</span>`
        : '';
      const rmHtml = !disabled
        ? `<button type="button" class="wc-file-upload-remove btn btn-sm" aria-label="Remove file">&times;</button>`
        : '';
      item.innerHTML = `
        ${media}
