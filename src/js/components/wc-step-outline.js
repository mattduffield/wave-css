...
if (!customElements.get('wc-step-outline')) {
  class WcStepOutline extends WcBaseComponent {
    ...
    constructor() {
      ...
      this._pausedStepNames = new Set(); // step names with breakpoint toggled on
    ...
    _loadPausedSteps() {
      ...
      } catch (_) { /* malformed storage — ignore, start fresh */ }
    }
    ...
    _render() {
      ...
      for (let i = 0; i < ranges.length; i++) {
        ...
        // Pause-breakpoint toggle — sets/clears a "pause here" flag for
        // this step. State persists per-pilot in localStorage (when the
        // outline has a storage-key attribute). The Debug button's
        // request payload includes the set; the macro pauses on match.
        ...
        if (stepName) {
          ...
          pauseBtn.addEventListener('click', function (e) {
            e.stopPropagation(); // don't fire the row's jump-to-line
